import XCTest
@testable import CanningLog

final class CanningLogTests: XCTestCase {
    ...
    func test_addEntry_increasesCount() {
        ...
        let entry = Entry(recipeName: "Test", date: Date(), note: "note")
    ...
    func test_addEntry_blockedAtFreeLimit() {
        ...
        while store.entries.count < Store.freeLimit {
            _ = store.add(Entry(recipeName: "Filler", date: Date(), note: ""), isPro: false)
        }
        let result = store.add(Entry(recipeName: "Overflow", date: Date(), note: ""), isPro: false)
    ...
    func test_addEntry_proBypassesLimit() {
        ...
        let result = store.add(Entry(recipeName: "ProEntry", date: Date(), note: ""), isPro: true)
    ...
    func test_deleteEntry_removesFromList() {
        let store = Store()
        let entry = Entry(recipeName: "ToDelete", date: Date(), note: "")
        _ = store.add(entry, isPro: false)
        store.delete(entry)
    ...
    func test_updateEntry_changesFields() {
        let store = Store()
        var entry = Entry(recipeName: "Original", date: Date(), note: "")
        _ = store.add(entry, isPro: false)
        entry.recipeName = "Updated"
        store.update(entry)
        XCTAssertEqual(store.entries.first(where: { $0.id == entry.id })?.recipeName, "Updated")
    }
