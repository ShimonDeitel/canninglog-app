...
struct EntryFormView: View {
    ...
    init(entry: Entry?, onSave: @escaping (Entry) -> Void) {
        _fieldValue = State(initialValue: entry?.recipeName ?? "")
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                Form {
                    Section("Recipe name") {
                        TextField("Recipe name", text: $fieldValue)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        let entry = Entry(
                            id: entryID ?? UUID(),
                            recipeName: fieldValue.isEmpty ? "Untitled" : fieldValue,
                            date: date,
                            note: note
                        )
                        onSave(entry)
